namespace :setup do
    ...
    task :install_requirements_server do
      on roles(:app) do
        ...
        execute "sudo apt-get -y install python3 python3-pip python-dev python3-dev python-virtualenv libpq-dev nginx supervisor git ssh libjpeg-dev zlib1g-dev libpng12-dev curl"
    ...
    task :install_requirements do
      on roles(:app) do
        ...
        execute "#{fetch(:deploy_to)}/bin/pip3 install -r #{fetch(:deploy_to)}/current/requirements_production.txt"
      end
    end
    task :"migrations" do
      on roles(:app) do
        ...
        execute "source #{fetch(:deploy_to)}/bin/activate"
        execute "#{fetch(:deploy_to)}/bin/python3 #{fetch(:deploy_to)}/current/manage.py migrate --settings=#{fetch(:application)}.settings_production"
      end
    end
    task :"conf_files" do
      on roles(:app) do
    ...
    task :collect_static do
      on roles(:app) do
        execute "echo '==================== Collectstatic ===================='"
         execute "#{fetch(:deploy_to)}/bin/python3 #{fetch(:deploy_to)}/current/manage.py collectstatic -v0 --noinput --settings=#{fetch(:application)}.settings_production"
      end
    end
end